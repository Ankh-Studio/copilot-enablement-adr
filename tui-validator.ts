...
/**
 * TUI Validator - Terminal User Interface for Enhanced AI Evaluation
 *
 * Interactive terminal interface for pasting client ADR content and getting instant validation
 * Uses the EnhancedOutputValidator with persona-based scoring and advanced AI techniques
 */
...
function printHeader(): void {
  console.log(
    `${colors.bgBlue}${colors.white}    🧠 ENHANCED AI EVALUATION TUI    ${colors.reset}`
  );
  console.log(
...
function printPersonaResult(
  ...
  console.log(
    `${colors.cyan}🎭 ${persona}:${colors.reset} ${scoreColor}${score}/100${colors.reset} ${status}`
  );
...
function printAdvancedEvaluation(results: any): void {
...
class TUIValidator {
  ...
  async start(): Promise<void> {
    ...
    printInfo(
      'This tool uses advanced AI techniques to evaluate client ADR content.'
    );
  ...
  private displayResults(results: any): void {
    ...
    printAdvancedEvaluation(results);

    // Recommendations
    printRecommendations(results.recommendations);
